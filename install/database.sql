...
-- ------------------------------------------------------------
-- SEO: cities, pages, keywords, blog, redirects, 404 log
-- ------------------------------------------------------------
CREATE TABLE `{PREFIX}seo_cities` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `city_name` VARCHAR(120) NOT NULL, `city_name_gu` VARCHAR(120) DEFAULT NULL,
  `state` VARCHAR(80) DEFAULT NULL, `slug` VARCHAR(120) NOT NULL,
  `population_tier` TINYINT DEFAULT 3, `meta_title` VARCHAR(200) DEFAULT NULL,
  `meta_description` VARCHAR(300) DEFAULT NULL, `h1` VARCHAR(200) DEFAULT NULL,
  `content_html` MEDIUMTEXT DEFAULT NULL, `hero_image` VARCHAR(255) DEFAULT NULL,
  `latitude` DECIMAL(10,6) DEFAULT NULL, `longitude` DECIMAL(10,6) DEFAULT NULL,
  `is_active` TINYINT(1) NOT NULL DEFAULT 1, `priority` INT NOT NULL DEFAULT 0,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`), UNIQUE KEY `uq_city_slug` (`slug`),
  KEY `idx_city_state` (`state`), KEY `idx_city_active` (`is_active`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE `{PREFIX}seo_pages` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `page_key` VARCHAR(80) NOT NULL,
  `url_path` VARCHAR(190) NOT NULL, `meta_title` VARCHAR(200) DEFAULT NULL,
  `meta_description` VARCHAR(300) DEFAULT NULL, `h1` VARCHAR(200) DEFAULT NULL,
  `canonical` VARCHAR(255) DEFAULT NULL, `robots` VARCHAR(40) DEFAULT 'index,follow',
  `og_image` VARCHAR(255) DEFAULT NULL, `schema_json` MEDIUMTEXT DEFAULT NULL,
  `is_active` TINYINT(1) NOT NULL DEFAULT 1,
  PRIMARY KEY (`id`), UNIQUE KEY `uq_page_key` (`page_key`), KEY `idx_page_path` (`url_path`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE `{PREFIX}seo_keywords` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `keyword` VARCHAR(190) NOT NULL,
  `type` VARCHAR(30) DEFAULT 'primary', `target_url` VARCHAR(255) DEFAULT NULL,
  `search_volume` INT DEFAULT NULL, `current_rank` INT DEFAULT NULL,
  `last_checked` DATE DEFAULT NULL, `notes` VARCHAR(255) DEFAULT NULL,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`), KEY `idx_kw_type` (`type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE `{PREFIX}blog_categories` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `name` VARCHAR(120) NOT NULL,
  `slug` VARCHAR(120) NOT NULL, `description` VARCHAR(300) DEFAULT NULL,
  `meta_title` VARCHAR(200) DEFAULT NULL, `meta_description` VARCHAR(300) DEFAULT NULL,
  PRIMARY KEY (`id`), UNIQUE KEY `uq_bcat_slug` (`slug`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE `{PREFIX}blog_posts` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `title` VARCHAR(200) NOT NULL,
  `slug` VARCHAR(200) NOT NULL, `excerpt` VARCHAR(400) DEFAULT NULL,
  `content` MEDIUMTEXT DEFAULT NULL, `featured_image` VARCHAR(255) DEFAULT NULL,
  `image_alt` VARCHAR(255) DEFAULT NULL, `category_id` INT UNSIGNED DEFAULT NULL,
  `tags` VARCHAR(255) DEFAULT NULL, `meta_title` VARCHAR(200) DEFAULT NULL,
  `meta_description` VARCHAR(300) DEFAULT NULL, `author` VARCHAR(120) DEFAULT NULL,
  `status` ENUM('draft','published','scheduled') NOT NULL DEFAULT 'draft',
  `published_at` DATETIME DEFAULT NULL, `views` INT UNSIGNED NOT NULL DEFAULT 0,
  `reading_time` INT DEFAULT NULL, `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`), UNIQUE KEY `uq_post_slug` (`slug`),
  KEY `idx_post_status` (`status`), KEY `idx_post_cat` (`category_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE `{PREFIX}redirects` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `from_url` VARCHAR(255) NOT NULL,
  `to_url` VARCHAR(255) NOT NULL, `type` ENUM('301','302') NOT NULL DEFAULT '301',
  `hits` INT UNSIGNED NOT NULL DEFAULT 0, `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`), UNIQUE KEY `uq_from` (`from_url`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE `{PREFIX}error_404_log` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `url` VARCHAR(255) NOT NULL,
  `referrer` VARCHAR(255) DEFAULT NULL, `user_agent` VARCHAR(255) DEFAULT NULL,
  `ip` VARCHAR(45) DEFAULT NULL, `hits` INT UNSIGNED NOT NULL DEFAULT 1,
  `last_seen` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`), UNIQUE KEY `uq_404_url` (`url`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ------------------------------------------------------------
-- AI token usage & cost metering
-- ------------------------------------------------------------
CREATE TABLE `{PREFIX}ai_usage_logs` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL, `username` VARCHAR(100) DEFAULT NULL,
  `request_id` VARCHAR(64) DEFAULT NULL, `created_at` DATETIME NOT NULL,
  `provider` VARCHAR(32) DEFAULT NULL, `model_name` VARCHAR(80) DEFAULT NULL,
  `input_tokens` INT DEFAULT 0, `output_tokens` INT DEFAULT 0,
  `thinking_tokens` INT DEFAULT 0, `cached_tokens` INT DEFAULT 0, `total_tokens` INT DEFAULT 0,
  `input_cost_usd` DECIMAL(14,10) DEFAULT 0, `output_cost_usd` DECIMAL(14,10) DEFAULT 0,
  `cached_cost_usd` DECIMAL(14,10) DEFAULT 0, `total_cost_usd` DECIMAL(14,10) DEFAULT 0,
  `total_cost_local` DECIMAL(14,4) DEFAULT 0,
  `key_owner` ENUM('platform','user') DEFAULT 'platform',
  `source` VARCHAR(40) DEFAULT NULL, `reference_id` VARCHAR(80) DEFAULT NULL,
  `status` ENUM('success','error','blocked','incomplete') DEFAULT 'success',
  `error_message` TEXT NULL, `response_time_ms` INT DEFAULT 0,
  PRIMARY KEY (`id`),
  KEY `idx_user_date` (`user_id`,`created_at`), KEY `idx_created` (`created_at`),
  KEY `idx_owner_date` (`key_owner`,`created_at`), KEY `idx_status_date` (`status`,`created_at`),
  KEY `idx_user_status_date` (`user_id`,`status`,`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE `{PREFIX}ai_pricing_config` (
  `id` INT AUTO_INCREMENT PRIMARY KEY, `provider` VARCHAR(32) NOT NULL,
  `model_name` VARCHAR(80) NOT NULL, `input_per_million` DECIMAL(12,6) DEFAULT 0,
  `output_per_million` DECIMAL(12,6) DEFAULT 0, `cached_per_million` DECIMAL(12,6) DEFAULT 0,
  `is_active` TINYINT(1) DEFAULT 1, `notes` VARCHAR(255) DEFAULT NULL, `updated_at` DATETIME DEFAULT NULL,
  UNIQUE KEY `uq_provider_model` (`provider`,`model_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE `{PREFIX}ai_usage_daily` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, `user_id` INT NOT NULL, `usage_date` DATE NOT NULL,
  `calls` INT DEFAULT 0, `input_tokens` BIGINT DEFAULT 0, `output_tokens` BIGINT DEFAULT 0,
  `total_tokens` BIGINT DEFAULT 0, `cost_usd` DECIMAL(14,6) DEFAULT 0, `cost_local` DECIMAL(14,4) DEFAULT 0,
  PRIMARY KEY (`id`), UNIQUE KEY `uq_user_date` (`user_id`,`usage_date`), KEY `idx_daily_date` (`usage_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
